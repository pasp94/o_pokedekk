...
//         
//         strongSelf.apiManager.getImage(urlString: "", completion: { [weak self] (sprite, error) in
//            guard let strongSelf = self else { return }
//            
//            guard error == nil,
//                  let image = sprite
...
//      apiManager?.getDetail(urlString: detailURL, decodingType: PokemonDetails.self, completion: { (pokemonDetail, error) in
//         guard error == nil,
//               let pokemonValue = pokemonDetail
//         else { /*show message error in view*/return }
//
//         self.pokemon.value = pokemonValue.toPokemon(with: Data())
//      })
   }
}
